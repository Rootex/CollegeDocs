...
def count_transitions(taggedWords):
    ...
    bigrams_cp = list(bigrams)
    ''' TODO: Fill (or re-implement) this function '''
    for key in countTransitionsOfPostags:
        for i in distinctPostags:
            countTransitionsOfPostags[key][i] = 0
    for key in countTransitionsOfPostags:
        for key1 in countTransitionsOfPostags[key]:
            tup = tuple([key, key1])
            countTransitionsOfPostags[key][key1] = bigrams_cp.count(tup)

    for key in countPostagsPerPostag:
        count = 0
        for i in bigrams_cp:
            if i[0] == key:
                count += 1
        countPostagsPerPostag[key] = count
...
def count_emissions(taggedWords):
    ...
    #print(countEmissionsOfPostags)
    #print(allCounts)
    #print(countWordsPerPostag)
    ''' TODO: Fill (or re-implement) this function '''
    ...
    for key in countEmissionsOfPostags:
        for i in tokens:
            tup = tuple([i, key])
            if tup in allCounts:
                countEmissionsOfPostags[key][i] = 0
    #print(countEmissionsOfPostags)
    for key in countEmissionsOfPostags:
        for key1 in countEmissionsOfPostags[key]:
            tup = tuple([key1, key])
            count = 0
            countEmissionsOfPostags[key][key1] = allCounts[tup]
    for i in postags:
        count = 0
        for key, value in allCounts.items():
            if key[1] == i:
                count += value
        countWordsPerPostag[i] = count
...
def get_transition_probability(previousPostag, countDataTransitions):
    ...
    for i in countTransitionsOfPostags[previousPostag]:
        probabilityTransitions[i] = countTransitionsOfPostags[previousPostag][i]/\
                                    float(countPostagsPerPostag[previousPostag])
...
def HMM_predict(symbols, get_TP, get_EP, probabilityState0):
    ...
    def compute_matrices(mapSymbols, mapStates):
        ...
        for state in mapStates['all']:
            ...
            #newStates = tuple([x for x in newStates if x != '.'])
            #print(newStates)
...
if __name__ == '__main__':
    ...
    taggedWords = {
        'train': read_tagged_file(DATA_PATH + 'brown-train.txt'),
        'development': read_tagged_file(DATA_PATH + 'brown-development.txt'),
        'test': read_tagged_file(DATA_PATH + 'brown-test.txt')
    }
    ...
    def evaluate_HMM_for_epsilon(evaluateWords, evaluatePostags, OOVWords, epsilon, countSentences=False):
        ...
        #print(probabilityPostag0)
        predictedPostags, logProbability = HMM_predict(evaluateWords, get_TP, get_EP, probabilityPostag0)
        MCR, MCRRatioOOV, sentences = evaluate(evaluatePostags, predictedPostags, evaluateWords, OOVWords,
                                               countSentences)
    ...
    MCR, MCRRatioOOV, sentences, logProbability = evaluate_HMM_for_epsilon(testWords, testPostags, OOVtest, epsilonBest,
                                                                           countSentences=True)
