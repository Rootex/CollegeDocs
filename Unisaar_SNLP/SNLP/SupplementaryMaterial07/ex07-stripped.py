...
def sentence_score_f_a_given_e(sentenceE, sentenceF, alignment, tokenProbabilityFGivenE):
    ...
    for i in range(len(sentenceF)):
        prob += tokenProbabilityFGivenE[sentenceEN[alignment[i]]][sentenceF[i]] if alignment[i] == -1 \
            else tokenProbabilityFGivenE[sentenceEN[alignment[i]]][sentenceF[i]]
     # also consider log-scale probabilities.
    return exp(logprob if prob == 0 else logprob + log(prob))
...
def sentence_score_f_given_e(sentenceE, sentenceF, tFGENZ):
    ...
    '''
    for a in alignmentsValid:
        prob += sentence_score_f_a_given_e(sentenceE, sentenceF, a, tFGENZ)
     # also consider log-scale probabilities.
    return prob
...
def rank_valid_alignments(sentenceE, sentenceF, tEGFNZ):
    ...
    scoreAlignments = []
    for a in alignments:
        scoreAlignments.append((a, sentence_score_f_a_given_e(sentenceE, sentenceF, a, tEGFNZ)))
...
def rank_valid_translations(sentenceE, m, tEGFNZ):
    ...
    '''
    scoreSentences = []
    for tokene in sentenceE:
        for tupf in list(sentencesF):
            for tokenf in tupf:
                if tokenf in  tEGFNZ[tokene]:
                    scoreSentences.append(((tokene, tokenf), tEGFNZ[tokene][tokenf]))
    scoreSentences = set(scoreSentences)
